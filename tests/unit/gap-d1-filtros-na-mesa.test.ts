 ...
 * Por isso a guarda decisiva lê a seção "quem pode participar" da rota e exige o JSX.
 */
...
const ROTA = "src/app/portal-curador/casos/[id]/mesa/page.tsx";
...
/**
 * O corpo da seção "quem pode participar" — a porta da Curadoria.
 *
 * A Mesa antiga tinha slots (`PERFIL:`, `REDE:`) e o painel morava no slot do
 * Perfil. A Mesa nova (ADR-093) é um documento contínuo, e o filtro passou a
 * morar onde a CONSEQUÊNCIA dele é lida: acima da lista de quem sobrou.
 * Mesmo princípio que trouxe o juízo para dentro da célula.
 */
function blocoDaPorta(): string {
  const fonte = readFileSync(path.join(RAIZ, ROTA), "utf8");
  const ini = fonte.indexOf('id="quem-pode-participar"');
  const fim = fonte.indexOf("</section>", ini);
  expect(ini, "a seção 'quem pode participar' sumiu da rota").toBeGreaterThan(-1);
  expect(fim, "a seção 'quem pode participar' não fecha").toBeGreaterThan(ini);
...
describe("T-11-9 · os filtros são alcançáveis PELA ROTA", () => {
   ...
   */
  it("a Mesa RENDERIZA o painel na porta da Curadoria", () => {
    expect(
      blocoDaPorta(),
      "o JSX de MandatoryFilters saiu da seção de quem pode participar — importar não é renderizar, " +
