     ...
     or exists (
       ...
       where table_schema = 'curadoria'
         and (
           ...
           or (table_name = 'legal_document_versions' and column_name in (
             'variaveis_requeridas','assinantes_exigidos','escopos_revogaveis','nivel_exigido',
             'aprovado_por','aprovado_em','motivo_da_mudanca'
           ))
           or (table_name = 'legal_acceptances' and column_name in (
             'instance_id','signer_id','instancia_hash','natureza','registrado_por',
             'forma_de_obtencao','nivel','provedor','evidencia_externa','declaracao_de_vontade'
           ))
...
create or replace function curadoria.g0_r1_contratos_json_validos(
  _variaveis jsonb,
  _assinantes jsonb,
  _escopos jsonb
)
...
declare
  _e jsonb;
begin
  if jsonb_typeof(_variaveis) <> 'array'
     or jsonb_typeof(_assinantes) <> 'array'
     or jsonb_typeof(_escopos) <> 'array' then
    return false;
  end if;

  for _e in select value from jsonb_array_elements(_variaveis) loop
    if not (
      (jsonb_typeof(_e) = 'string' and length(btrim(_e #>> '{}')) > 0)
      or (
        jsonb_typeof(_e) = 'object'
        and jsonb_typeof(_e -> 'chave') = 'string'
        and length(btrim(_e ->> 'chave')) > 0
        and (_e - 'chave' - 'rotulo') = '{}'::jsonb
      )
  ...
  for _e in select value from jsonb_array_elements(_assinantes) loop
    if jsonb_typeof(_e) <> 'object'
       or jsonb_typeof(_e -> 'papel') <> 'string'
  ...
  for _e in select value from jsonb_array_elements(_escopos) loop
    if jsonb_typeof(_e) <> 'object'
       or jsonb_typeof(_e -> 'codigo') <> 'string'
       or length(btrim(_e ->> 'codigo')) = 0
       or jsonb_typeof(_e -> 'rotulo') <> 'string'
       or length(btrim(_e ->> 'rotulo')) = 0
...
alter table curadoria.legal_document_versions
  ...
  add constraint legal_document_versions_contratos_validos check (
    curadoria.g0_r1_contratos_json_validos(
      variaveis_requeridas, assinantes_exigidos, escopos_revogaveis
    )
  );
...
create table curadoria.legal_document_instances (
  ...
  constraint legal_instances_artefato_coerente check (
    ...
    or (
      length(btrim(artefato_tipo)) > 0
      and jsonb_typeof(artefato_ref) = 'object'
      and length(btrim(artefato_versao)) > 0
      and artefato_hash ~ '^[0-9a-f]{64}
);
...
begin
  ...
  if jsonb_typeof(coalesce(_variaveis, '{}'::jsonb)) <> 'object'
     or jsonb_typeof(coalesce(_contexto, '{}'::jsonb)) <> 'object' then
  ...
  if _artefato is not null and (
       jsonb_typeof(_artefato) <> 'object'
       or (_artefato - 'tipo' - 'ref' - 'versao' - 'hash') <> '{}'::jsonb
       or jsonb_typeof(_artefato -> 'tipo') <> 'string'
       or jsonb_typeof(_artefato -> 'ref') <> 'object'
       or jsonb_typeof(_artefato -> 'versao') <> 'string'
       or jsonb_typeof(_artefato -> 'hash') <> 'string'
       or length(btrim(_artefato ->> 'tipo')) = 0
       or length(btrim(_artefato ->> 'versao')) = 0
  ...
  -- Uma chave repetida com contrato diferente é colisão, nunca autorização.
  if _idempotency_key is not null then
    ...
    if found then
      if _instancia.version_id is distinct from _versao.id
         or _instancia.profile_id is distinct from _profile_id
         or _instancia.professional_profile_id is distinct from _professional_profile_id
         or _instancia.case_id is distinct from _case_id
         or _instancia.contexto is distinct from coalesce(_contexto, '{}'::jsonb)
         or _instancia.variaveis is distinct from coalesce(_variaveis, '{}'::jsonb)
         or _instancia.corpo is distinct from _corpo
         or _instancia.expira_em is distinct from _expira_em
         ...
         or _instancia.artefato_versao is distinct from (_artefato ->> 'versao')
         or _instancia.artefato_hash is distinct from (_artefato ->> 'hash')
      then
        raise exception 'Colisão de idempotência: a chave já representa outro contrato'
          using errcode = '23505';
      end if;
      return _instancia;
    end if;
  ...
  exception when unique_violation then
    if _idempotency_key is not null then
      ...
      where gerada_por = _actor and idempotency_key = _idempotency_key;
      if found
         and _instancia.version_id is not distinct from _versao.id
         and _instancia.profile_id is not distinct from _profile_id
         and _instancia.professional_profile_id is not distinct from _professional_profile_id
         and _instancia.case_id is not distinct from _case_id
         and _instancia.contexto is not distinct from coalesce(_contexto, '{}'::jsonb)
         and _instancia.variaveis is not distinct from coalesce(_variaveis, '{}'::jsonb)
         and _instancia.corpo is not distinct from _corpo
         and _instancia.expira_em is not distinct from _expira_em
         ...
         and _instancia.artefato_versao is not distinct from (_artefato ->> 'versao')
         and _instancia.artefato_hash is not distinct from (_artefato ->> 'hash')
      then
        return _instancia;
      end if;
      raise exception 'Colisão de idempotência: a chave já representa outro contrato'
...
begin
  ...
  if _provedor is not null or _evidencia_externa is not null then
    raise exception 'N3 exige porta confiável de provedor; o caminho público não aceita evidência autodeclarada'
      using errcode = '42501';
  ...
  if _versao.nivel_exigido is not null and not (
    ...
    raise exception 'Nível de assinatura % inferior ao exigido % para o assinante %',
      _nivel, _versao.nivel_exigido, _assinante.id
      using errcode = '23514';
  end if;
...
revoke all on function curadoria.g0_r1_contratos_json_validos(jsonb, jsonb, jsonb) from public, anon, authenticated;
  ...
  constraint legal_instances_idempotencia_nao_vazia check (
...
-- Idempotência só onde declarada.
create unique index legal_instances_idempotencia
  on curadoria.legal_document_instances (idempotency_key)
  where idempotency_key is not null;
...
begin
  ...
  -- Idempotência SÓ quando declarada: sem chave, duas chamadas são dois
  -- documentos — que é o que duas propostas de contrato de fato são.
  if _idempotency_key is not null then
    select * into _instancia
    from curadoria.legal_document_instances
  ...
  if jsonb_typeof(coalesce(_variaveis, '{}'::jsonb)) <> 'object' then
    raise exception 'As variáveis do instrumento são um objeto' using errcode = '23514';
  end if;
  ...
  -- A verificação de idempotência acima resolve a repetição SEQUENCIAL (o
  -- cliente que reenvia após timeout). A corrida — duas chamadas simultâneas
  -- com a mesma chave — só o índice único resolve, e quem perde precisa
  -- receber a instância vencedora, nunca um erro: idempotência que falha sob
  -- concorrência não é idempotência. Sem estado parcial: ou a linha inteira
  -- entra com seus assinantes, ou a transação não deixa rastro.
...
begin
  ...
  -- O NÍVEL É DO SERVIDOR. O cliente não o informa e não o escolhe: ele sai
  -- do caminho efetivamente percorrido.
  if _provedor is not null and _evidencia_externa is not null then
    _nivel := 'N3';
  elsif _natureza = 'eletronico_pelo_titular'
        and _declaracao_de_vontade is not null then
    select btrim(coalesce(p.display_name, '')) into _nome_cadastro
...
revoke all on function curadoria.enforce_legal_signers_congelados() from public, anon, authenticated;
    ...
    raise exception 'Artefato inválido: tipo, ref-objeto, versão e hash SHA-256 são obrigatórios'
      using errcode = '23514';
  end if;
