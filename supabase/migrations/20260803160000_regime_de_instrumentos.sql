-- G0.1 — REGIME DE INSTRUMENTOS: o documento que só existe preenchido.
--
...
alter table curadoria.legal_document_versions
  ...
  add constraint legal_document_versions_contratos_sao_listas check (
    jsonb_typeof(variaveis_requeridas) = 'array'
    and jsonb_typeof(assinantes_exigidos) = 'array'
    and jsonb_typeof(escopos_revogaveis) = 'array'
  );
...
create table curadoria.legal_document_instances (
  ...
  constraint legal_instances_artefato_coerente check (
    (artefato_tipo is null and artefato_ref is null and artefato_hash is null)
    or (artefato_tipo is not null and artefato_ref is not null and artefato_hash is not null)
  ),
...
-- estas funções (elas recusariam por falta de auth.uid(), mas a superfície
