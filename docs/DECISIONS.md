...
- **Decisão:**
  ...
  3. **Grafo de transições fechado, com cinco transições permitidas:** `inexistente→PROPOSTA` · `PROPOSTA→VIGENTE` · `PROPOSTA→SUSPENSA` · `VIGENTE→SUSPENSA` · `VIGENTE→REVOGADA` · `SUSPENSA→VIGENTE` · `SUSPENSA→REVOGADA`. **`REVOGADA` é terminal.** **`PROPOSTA→REVOGADA` não existe** — não se revoga o que nunca valeu; o ato correto é `PROPOSTA→SUSPENSA`. **Nenhuma versão volta a `PROPOSTA`.** Ciclos sucessivos `VIGENTE ↔ SUSPENSA` são ilimitados, e contá-los é sinal de calibração.
