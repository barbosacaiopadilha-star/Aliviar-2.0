...
- **Registro de implementação (dependência, 2026-09-03) — não altera esta decisão.** A dependência `@anthropic-ai/sdk` foi removida do `package.json` em 2026-09-03: nenhum arquivo de `src/`, `scripts/` ou `tests/` a importava desde a aposentadoria do rascunho assistido/ACE (auditoria SIM-102, `docs/REGISTRO_UNICO_DE_ACHADOS.md`), e a Anthropic deixa de ser suboperadora ativa no código.
