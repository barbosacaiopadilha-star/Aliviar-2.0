...
| **2.3** | Divisão da etapa AVALIAÇÃO | 3 critérios do lado da pessoa passam a vir do Motor | **ADR-067** (ex-"ADR-B") · 1.4 ✓ · **2.4 (dependência MATERIAL — os juízos H8–H11 têm `curator_judgments` como destino; CONTRATO_2_4 §18)** | X1 verde; FORMACAO/EXPERIENCIA/HISTORICO permanecem humanos | **AGUARDA 2.4** — dependência material lavrada em 2026-08-08; pré-voo próprio só após o 2.4 encerrado | Implementador | flag restaura 6×N | L-40 + L-41 |
| **2.4** | `curator_judgments` **sem `AREA`** | Registrar juízo humano onde não há célula | **ADR-067** (ex-"ADR-B"), 2.1 | Append-only; `AREA` ausente (RS-03) | **CONTRATO APROVADO — APTO A RECEBER MISSÃO DE IMPLEMENTAÇÃO** (Guardião, 2026-08-08, PA-15; ressalva evidência×julgamento incorporada; CHECK, nunca FK ao Catálogo; "mesmo conteúdo" definido) — [`CONTRATO_2_4_CURATOR_JUDGMENTS.md`](CONTRATO_2_4_CURATOR_JUDGMENTS.md); **implementação não iniciada** | Implementador | migration aditiva | **novo** |
