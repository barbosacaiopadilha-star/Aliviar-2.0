...
| Código | Nome | Objetivo | Dep. | Aceite | Estado | Resp. | Rollback | Era |
|---|---|---|---|---|---|---|---|---|
| **1.8** | **Ficha de Explicação** (§11) | Explicabilidade obrigatória, incluindo explicação de proposta | 1.1 · **1.9** (ver §2.3) | **AC-EXPLICA** (bloqueante): sem explicação, a superfície **não renderiza**; fallback genérico proibido | **ABERTO — aguarda o `1.8-R1-MR1`** · `c3242ea` base válida; §11.4 **satisfeito em `041b423`** (12/12, Agente 04); encerra só com o microcorretivo verificado (Contrato §22.10) | Implementador | remover a Ficha desliga o consumo, não a regra | **novo** |
| **1.8-R1** | **Fechamento da Proveniência da Ficha** | Cumprir integralmente o §11.4 | 1.8 · 1.9 · [`CONTRATO_1_8_R1.md`](CONTRATO_1_8_R1.md) | os **doze** critérios do §16 do Contrato | **VERIFICADO COM RESSALVAS** (`041b423`) — 12/12 critérios confirmados pelo Agente 04; capability e inércia sem ressalvas; regressão verde. **Aguarda o microcorretivo de encerramento `1.8-R1-MR1`** (Contrato §22) | Implementador | reverter ao estado de `c3242ea`; as migrations são aditivas e reversíveis | **novo** |
| **1.8-R1-MR1** | **Microcorretivo de encerramento** (Micro-retificação) | Trigger recusa `NULL` na mudança de `status` de linha vinculada (§22.4) · seis testes do trigger · teste concorrente · Ficha sai de `INERTES_AUTORIZADOS` | 1.8-R1 · Contrato §22 | §22.6 + §22.7 verdes · A5/A5b fechados com a lista reduzida · `041b423` segue verde | **LAVRADO — aguarda autorização do DT-01** | Implementador | reverter a migration nova do trigger; a lista A5 volta com um `git revert` | **novo** |
