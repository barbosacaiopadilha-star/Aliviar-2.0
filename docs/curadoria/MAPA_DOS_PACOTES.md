...
| **1.8-R1** | **Fechamento da Proveniência da Ficha** | Cumprir integralmente o §11.4 | 1.8 · 1.9 · [`CONTRATO_1_8_R1.md`](CONTRATO_1_8_R1.md) | os **doze** critérios do §16 do Contrato | **EM EXECUÇÃO** — checkpoint: migration do vínculo aplicada e conferida (não rastreada) · C-01/C-01b/C-01c e `AUSENTE`×`NAO_APLICAVEL` implementados · A1.1 escrito, vermelho pela inércia da 2.1 · **leitor controlado lavrado em 2026-08-07 (Contrato §21): capability `SECURITY DEFINER` p/ `service_role`, C-01 volta a proibir a tabela em todo `src/`, migration nova (caminho B) — A1.1 desbloqueada para a retomada** | Implementador | reverter ao estado de `c3242ea`; as migrations são aditivas e reversíveis | **novo** |
