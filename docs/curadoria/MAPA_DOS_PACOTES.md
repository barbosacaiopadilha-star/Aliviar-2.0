...
| **2.6** | Governança de quem confirma o Mapa do Profissional | Corrigir G4/RI4 | **ADR-068** (ex-"ADR-D"; toca ADR-040 item 6) | RLS não enfraquece; teste de permissão por papel | **CONTRATO APROVADO — APTO A RECEBER MISSÃO DE IMPLEMENTAÇÃO** (Guardião, 2026-08-08, PA-14; **G-10 = Opção B**; catálogo de 3 desfechos; gate-first): objeto residual pós-ADR-068 §14.2 + Item 1.2 incorporado; **não implementado**; 2.C segue bloqueado; ver [`CONTRATO_2_6_RELEITURA_DE_ESCOPO.md`](CONTRATO_2_6_RELEITURA_DE_ESCOPO.md); decisão principal: G-10 (A×B×C, recomendação B) | Implementador + Guardião | restaurar permissão anterior | L-23 |
