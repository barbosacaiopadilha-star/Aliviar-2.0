...
| **2.3** | Divisão da etapa AVALIAÇÃO | 3 critérios do lado da pessoa passam a vir do Motor | **ADR-067** (ex-"ADR-B") · 1.4 ✓ · **2.4 (dependência MATERIAL — os juízos H8–H11 têm `curator_judgments` como destino; CONTRATO_2_4 §18)** | X1 verde; FORMACAO/EXPERIENCIA/HISTORICO permanecem humanos | **CONTRATO APROVADO — APTO A RECEBER MISSÃO DE IMPLEMENTAÇÃO** (Guardião, 2026-08-08, PA-16; **RS-2.3-1 incorporada**: retirada exige Curador do Case **e** autor da versão vigente; O-1/O-2 como notas) — [`CONTRATO_2_3_DIVISAO_DA_AVALIACAO.md`](CONTRATO_2_3_DIVISAO_DA_AVALIACAO.md); **implementação não iniciada** | Implementador | flag restaura 6×N | L-40 + L-41 |
