...
| **2.C** | Persistência e apresentação da derivação do Mapa do Profissional — **confirmação item a item** | Ligar 1.A ao mundo real | 1.A, 2.1, 2.6, **todas as dez do §15.0** | Fronteira Humana com os nove elementos (A2c); ausência de ato nunca confirma (A2d) | **CONTRATO EM PROPOSTA** (2026-08-08) — gates 10/10 auditados ([`AUDITORIA_GATES_2C.md`](AUDITORIA_GATES_2C.md)); [`CONTRATO_2_C_FRONTEIRA_DO_MAPA_DO_PROFISSIONAL.md`](CONTRATO_2_C_FRONTEIRA_DO_MAPA_DO_PROFISSIONAL.md) **aguarda julgamento do Guardião — é nele que a abertura da Fronteira se decide**; implementação não autorizada; **Fronteira segue FECHADA** | Implementador | flag desliga a superfície; preenchimento manual permanece | L-21 + L-22 |
