...
| **C-01d** A capability tem um chamador só *(1.8-R1 — a implementar na retomada)* | Só a lista nominal invoca `ler_proposta_para_proveniencia`; **nenhuma segunda função SQL expõe propostas** — o conjunto que alcança a tabela é `{emissor (C-11), leitora}` | Contrato §21.7 | `src/**` · `supabase/migrations/**` | um módulo fora da lista chama a capability · nasce uma terceira função sobre a tabela | detecção em **função pura**, falseável com entrada sintética (§18.5) |
