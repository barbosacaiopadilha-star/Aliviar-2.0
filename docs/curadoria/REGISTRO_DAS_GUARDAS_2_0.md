...
| Guarda | Objetivo | Princípio | Arquivos | Como falha | Como é detectada |
|---|---|---|---|---|---|
| **C-01** Nenhuma proposta persistida é **consumida** *(emendada em 2026-08-07)* | A estrutura nasce inerte; **nenhum módulo de produção consome** propostas persistidas, **exceto o repositório canônico de proveniência, que pode somente lê-las** para reconstrução e auditoria | P-08; Arquitetura §15.0 | `src/**` · `supabase/migrations/**` | a tabela ganha policy/grant · um estado `PROPOSTA` é persistido · **um módulo fora da lista nominal menciona `derivation_proposals`** | varredura do repositório, com exceção nominal por `LEITORES_DE_PROPOSTA_AUTORIZADOS` |
| **C-01b** O leitor autorizado é inerte e read-only *(1.8-R1)* | Autorizar a leitura não autoriza o consumo | [`CONTRATO_1_8_R1.md`](CONTRATO_1_8_R1.md) §18 | os arquivos de `LEITORES_DE_PROPOSTA_AUTORIZADOS` | o autorizado ganha `.insert`/`.update`/`.delete`/`.upsert`, RPC de escrita, emissão, seleção de regra, cálculo do Motor ou `export` de função de decisão | varredura do arquivo autorizado, com a **mesma lista** que isenta na C-01 |
...
| Guarda | Objetivo | Princípio | Arquivos | Como falha | Como é detectada |
|---|---|---|---|---|---|
| **D-01** Uma origem por fato | Cada entrada do Motor tem **um** escritor | P-07; Arquitetura §4.2 | `mapa-prioridades-repository.ts` · `mapa-profissional-repository.ts` | um segundo módulo passa a gravar um Mapa | detector de escrita por cadeia `from(...)` |
