...
| **C-01d** Cada capability tem um chamador só *(1.8-R1; evoluída em 2026-08-07 pelo 1.11; **evolução para quatro aprovada — PA-12; **evolução para CINCO lavrada — RS-2.C-1, PA-17, 2026-08-08**)* | Capabilities nominais com chamadores **distintos**: `ler_proposta_para_proveniencia` → `cadeia-de-proveniencia-repository.ts` · `contar_propostas_por_desfecho` (1.11) → `painel-de-discordancia-repository.ts` · **aprovada (CONTRATO_1_12 §14): `decidir_proposta`** (decisora, invocada pelo cliente autenticado do Curador). O conjunto SQL autorizado sobre a tabela passa a **CINCO** funções nominais — `{emissor Case (C-11) · **emissor profissional `emitir_proposta_de_estado` (2.C, RS-2.C-1)** · leitora individual · leitora agregada · decisora (1.12)}` — um **sexto** nome derruba, e a guarda cai igualmente se a enumeração nominal for trocada por wildcard/prefixo/critério aberto. **Evolução nominal autorizada, não flexibilização** (anterior: quatro; razão: nascimento do emissor profissional pelo 2.C; autoridade: PA-17). **EVOLUÍDA, IMPLEMENTADA E VERIFICADA** (2026-08-08, Item 2.C `42bca9b`): as **cinco** existem e a enumeração nominal fechada está ativa — o sexto nome derruba. O caráter nominal fechado permanece; nenhuma flexibilização por wildcard/prefixo foi introduzida | Contrato 1.8 §21.7 · [`CONTRATO_1_11`](CONTRATO_1_11_PAINEL_DE_DISCORDANCIA.md) §10 · [`CONTRATO_1_12`](CONTRATO_1_12_MECANISMO_DE_DISCORDANCIA.md) §14 | `src/**` · `supabase/migrations/**` | um módulo fora da lista chama qualquer capability · chamador cruzado · nasce função fora do conjunto · **agregação por chamadas repetidas à individual** (§10.1) | detecção em **função pura**, falseável com entrada sintética |
...
### Guardas do Item 2.C — **IMPLEMENTADAS E VERIFICADAS** (PA-17; Item 2.C formalmente encerrado — `42bca9b`, 2026-08-08)

Lavradas no [`CONTRATO_2_C_FRONTEIRA_DO_MAPA_DO_PROFISSIONAL.md`](CONTRATO_2_C_FRONTEIRA_DO_MAPA_DO_PROFISSIONAL.md)
§14–§15; implementadas no pacote de abertura (`42bca9b`), verificadas de forma
independente pelo Agente 04 e certificadas pelo Agente 05 em 2026-08-08. **As
doze permanecem individualizadas:** G-2.C-1 (A2d) · G-2.C-2 (abertura exata) ·
G-2.C-3 (item a item) · G-2.C-4 (A2c) · G-2.C-5 (autoria/data/desfecho) ·
G-2.C-6 (Motor não confirma; confirmação não cria julgamento) · G-2.C-7 (DP-5) ·
G-2.C-8 (emissor limitado) · G-2.C-9 (CD-1) · G-2.C-10 (proveniência) ·
G-2.C-11 (separação de papéis) · G-2.C-12 (rollback sem apagar fatos).
**Ressalva vinculada `F-2.C-1` (higiene não bloqueante):** a varredura
**estática** de CD-1 — zero semeadura material — lê a migration do 2.C atual e
não capturaria, sozinha, uma migration futura que introduzisse seed de
correspondência; hoje `derivation_rule_degree_map` está **vazia** e CD-1 está
materialmente preservada. Nenhum pacote que toque a ponte pode ser aprovado sem
reproduzir o oráculo de CD-1 sobre a definição efetiva.

> **Padrão transversal registrado (não consolidado aqui):** `F-2.3-1`,
> `F-2.4-1`, `F-2.6-1` e `F-2.C-1` são, em essência, o mesmo achado — **alcance
> estático de guardas sobre artefatos futuros**. As quatro permanecem vigentes e
> individualmente rastreadas. A consolidação técnica, se desejada, merece
> **missão própria de higiene**, com autoridade específica; ela **não bloqueia**
> nenhum encerramento já lavrado.

| Guarda | Objetivo |
|---|---|
| **G-2.C-1** | A2d executável — decurso/navegação/fim de sessão deixam `PROPOSTA` e Mapa intacto |
| **G-2.C-2** | abertura exata — só o `EXECUTE` da decisora; PUBLIC/anon revogados; zero policy/grant de tabela |
| **G-2.C-3** | confirmação **item a item** — um `proposal_id` por ato; nenhum atalho de lote |
| **G-2.C-4** | os **nove elementos A2c** presentes — componente não decide sem eles |
| **G-2.C-5** | autoria/data/desfecho reais — sem ator de payload |
| **G-2.C-6** | Motor não confirma; confirmação não cria julgamento |
| **G-2.C-7** | DP-5 — nenhum regime de massa, nem equivalente |
| **G-2.C-8** | emissor profissional não escreve no Mapa, não julga, não confirma (A2b) |
| **G-2.C-9** | CD-1 — zero semeadura de regra/valor material |
| **G-2.C-10** | proveniência ponta a ponta — confirmação grava `evidence_id` da proposta |
| **G-2.C-11** | separação de papéis + §13.2 executável — paciente/Curador não confirmam o Mapa do Profissional |
| **G-2.C-12** | rollback não apaga fatos humanos |

### Guardas do Item 2.3 — **IMPLEMENTADAS E VERIFICADAS** (PA-16; Item 2.3 formalmente encerrado — `8305d97`, 2026-08-08)

Lavradas no [`CONTRATO_2_3_DIVISAO_DA_AVALIACAO.md`](CONTRATO_2_3_DIVISAO_DA_AVALIACAO.md)
§15–§16; implementadas no pacote técnico do 2.3 (`8305d97`), verificadas de forma
independente pelo Agente 04 (13 famílias de mutação mordidas, incluída a mutação
obrigatória da **RS-2.3-1** — não-autor retirando julgamento alheio ⇒ queda) e
certificadas pelo Agente 05 em 2026-08-08. **Ressalva vinculada `F-2.3-1`
(higiene não bloqueante, classe de `F-2.4-1`/`F-2.6-1`):** as varreduras
**estáticas** de assinatura sem `actor_id` e de ausência de destino paralelo
leem a migration lavrada do 2.3 e o `src/` atuais, e não capturariam, sozinhas,
uma migration futura que introduzisse deriva (overload aceitando ator; tabela
staging paralela); as camadas **vivas** pinam o regime efetivamente aplicado e
consumido. Nenhum pacote que altere as capabilities de julgamento ou seu destino
pode ser aprovado sem reproduzir os oráculos dessas guardas sobre a definição
efetiva. **F-2.4-1 permanece vigente e não é absorvida por esta.**
