...
| **C-01d** Cada capability tem um chamador só *(1.8-R1; **evoluída em 2026-08-07 pelo contrato do 1.11**)* | **Duas** capabilities autorizadas, com chamadores nominais **distintos**: `ler_proposta_para_proveniencia` → `cadeia-de-proveniencia-repository.ts` · `contar_propostas_por_desfecho` (agregada, 1.11) → `painel-de-discordancia-repository.ts`. O conjunto SQL que alcança a tabela é `{emissor (C-11) · leitora individual · leitora agregada}` — um quarto nome derruba | Contrato 1.8 §21.7 · [`CONTRATO_1_11`](CONTRATO_1_11_PAINEL_DE_DISCORDANCIA.md) §10 | `src/**` · `supabase/migrations/**` | um módulo fora da lista chama qualquer capability · chamador cruzado · nasce uma quarta função sobre a tabela · **agregação por chamadas repetidas à individual** (§10.1) | detecção em **função pura**, falseável com entrada sintética |
