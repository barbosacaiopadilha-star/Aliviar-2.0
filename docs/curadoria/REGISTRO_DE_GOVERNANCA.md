...
| ADRs da 2.0 | **ADR-066/067/068/069 lavradas**; ADR-C retirada; ADR-E recomendada (DP-2 já fechada sem ela). **ADR-070 — Aprovação da Regra Material 001, APROVADA E LAVRADA pelo DT-01 em 2026-08-08**: aprova o conteúdo da versão 1 de `CONTINUIDADE_COORDENACAO_CONDUTA_DECLARADA` e autoriza a entrada em `VIGENTE` pelo rito da ADR-069, com `approval_adr = 'ADR-070'`. **A regra existe em produção, em `PROPOSTA`** (`f1a7060`, ledger 114); **a promoção é ato próprio, ainda não praticado** | `DECISIONS.md`; anexo [`ADR_070_APROVACAO_DA_REGRA_001.md`](ADR_070_APROVACAO_DA_REGRA_001.md) |
