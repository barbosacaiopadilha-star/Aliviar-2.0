...
| 16g | [`CONTRATO_1_A_FUNCAO_PURA_DERIVACAO_MAPA_PROFISSIONAL.md`](CONTRATO_1_A_FUNCAO_PURA_DERIVACAO_MAPA_PROFISSIONAL.md) | Derivado | **PROPOSTA (2026-08-08) — aguarda aprovação do Guardião** | Item 1.A: função pura total com quatro braços (`FORA_DA_DERIVACAO`/`NAO_SUPORTADO`/`LACUNA`/`PROPOSTO`) · v1 = mecânica completa, derivação vazia — nenhuma semântica absoluta aprovada (ADR-065 é pairwise da leitura) · matriz dos 29 conceitos · P-04/I-8 executável (os três estados são afirmativos; ausência ⇒ LACUNA) · semântica futura só por regra versionada com lavratura própria (§10) |
