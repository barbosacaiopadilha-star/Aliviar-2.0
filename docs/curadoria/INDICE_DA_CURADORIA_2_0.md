...
| 16f | [`CONTRATO_1_12_MECANISMO_DE_DISCORDANCIA.md`](CONTRATO_1_12_MECANISMO_DE_DISCORDANCIA.md) | Derivado | **PROPOSTA (2026-08-08) — aguarda aprovação** | Mecanismo de discordância: tabela append-only de atos (`derivation_proposal_acts`) · capability única `decidir_proposta` com gate interno · confirmação atômica (ato + declaração no Mapa) · recusa = ato + lacuna · O2 desdobrado em O2-A..E · nasce inerte na Onda 1B (zero grants, zero superfície) · 1 pendência de autoridade (§18: visibilidade do motivo) |
