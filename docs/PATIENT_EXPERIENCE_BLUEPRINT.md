...
- **Decisões humanas existentes**: o Curador Médico decide `APPROVE`, `ADJUST`, `REJECT` ou `REQUEST_MORE_INFORMATION` (`submitHumanReviewAction`, ação server-side própria, nunca disparada pelo orquestrador automático).
...
- **Decisões humanas existentes**: Curador/Administrador aciona `deliverFinalCuradoriaAction` explicitamente — "nunca automático", mesmo após validação.
