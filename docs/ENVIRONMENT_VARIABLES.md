...
| `ANTHROPIC_API_KEY` | Condicional — obrigatória em produção | `src/modules/concierge/anthropic-language-model.ts`, selecionada por `getAceLanguageModel()` (`src/modules/concierge/language-model.ts`) | Ver seção "Seleção do modelo de linguagem" abaixo — o comportamento na ausência desta chave **depende do ambiente**, não é uniforme. |
...
| Ambiente | `ANTHROPIC_API_KEY` presente | `ANTHROPIC_API_KEY` ausente |
|---|---|---|
