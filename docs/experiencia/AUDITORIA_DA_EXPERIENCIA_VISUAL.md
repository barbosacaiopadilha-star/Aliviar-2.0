...
8. **Linguagem de sistema nas superfícies dela:** **zero** em texto de tela ✓ (única ocorrência é comentário de código). Nos fundos: "Endpoint preparado em POST", "Fila de Leads", "Visível por vínculo anterior".
   > **Correção factual (Onda 3).** Esta auditoria afirmou que o rodapé *"Ambiente de construção da experiência — dados de demonstração"* vazava em produção. **É falso:** `portal-shell.tsx:108` já o exclui por código quando `NODE_ENV === "production"`. Ele aparece apenas no servidor de desenvolvimento — que é de onde vieram as capturas. Nada a corrigir no produto.
