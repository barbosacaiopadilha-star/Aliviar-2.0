...
type CuradorCaseDetailPageProps = {
  params: Promise<{ id: string }>;
};

// Não usa createAdminSupabaseClient (service role) — o Curador Médico só
// enxerga o que a RLS já concede pelo cliente autenticado normal; por isso
// não exibimos e-mail aqui (só disponível via Admin API, reservada a
// administrador — src/lib/supabase/admin.ts).
export default async function CuradorCaseDetailPage({ params }: CuradorCaseDetailPageProps) {
  ...
  const [patientProfile, story, events, notes] = await Promise.all([
    getPatientProfile(supabase, caseDetail.patientProfileId),
    getStoryById(supabase, caseDetail.sourceStoryId),
    listCaseEvents(supabase, id),
    listCaseNotes(supabase, id),
  ]);
