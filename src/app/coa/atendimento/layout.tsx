...
export default async function CoaAtendimentoLayout({
  children,
}: Readonly<{ children: React.ReactNode }>) {
  const state = await getAuthState();
  if (!state) redirect("/login?next=/coa/atendimento");

  const allowed =
    canAccessCoaLevel(state.roles, "ATENDIMENTO") || state.roles.includes("administrador");
  ...
  return (
    <AppShell
      ...
      basePath="/coa/atendimento"
      systemLabel="COA · Atendimento"
    >
      {children}
    </AppShell>
  );
